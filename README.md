...
긴 웹툰 이미지를 일정 높이로 자르면 말풍선이 crop 경계에 걸려 YOLO26 추론모델이 박스를 놓치거나 잘린 박스를 반환할 수 있습니다. 예를 들어 `0~1600px`, `1600~3200px`처럼 딱 맞게 자르면 `1500~1750px` 영역의 말풍선은 두 crop 모두에서 일부가 잘릴 수 있습니다.

이를 줄이기 위해 crop height는 `1600px`, overlap은 `300px` 정도로 두고 `0~1600px`, `1300~2900px`, `2600~4200px`처럼 겹치게 분할하는 방식으로 설계했습니다. crop 상하단 `30~50px` 경계에 걸친 박스는 우선순위를 낮추거나 제외한 뒤, 원본 좌표로 합쳐 중복 박스를 제거합니다.
