# Webtoon 자동번역 모듈

웹툰 편집 워크플로우를 연습하기 위한 모노레포입니다. React 기반 프런트엔드와 Node.js 백엔드, 그리고 공용 타입을 한곳에 모았습니다.

## 폴더 구조
- `front-end` : 웹 편집기 UI (React, TypeScript)
- `back-end` : API 서버 및 OCR/번역 연동
- `shared` : 프런트엔드/백엔드에서 함께 쓰는 타입 및 유틸

## 빠른 시작
1) 필요한 의존성 설치: `back-end` 폴더에서 `npm install`  
2) 개발 서버 실행: `back-end`에서 `npm run start`
3) 브라우저에서 편집기 UI 접속 후 이미지 업로드 → 크롭/번역 흐름 테스트

## 주요 기능
- 이미지 위에 크롭 오버레이를 생성·이동
- OCR/번역 파이프라인 연습
- 편집 기록(Undo/Redo) 및 오버레이 저장

## 외부 API/라이브러리
- Google Translation API: 추출된 텍스트를 한국어/영어 등 목표 언어로 번역
- Tesseract OCR: 업로드된 웹툰 컷에서 텍스트 영역을 OCR로 인식
- Roboflow(자동 영역 탐지): 말풍선 등 텍스트가 위치한 영역을 자동 감지하여 크롭 시작점으로 활용
