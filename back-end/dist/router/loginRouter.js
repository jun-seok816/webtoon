...
loginRouter.post("/save_data_google", async (req, res) => {
    try {
        ...
        const emailNamePart = email.includes("@") ? email.substring(0, email.indexOf("@")) : email;
        const resolvedDisplayName = displayName ?? emailNamePart;
        /* ② users 테이블 조회 */
        const [rows] = await process._myApp.db
            .promise()
            .query("SELECT id, display_name FROM users WHERE email = ? LIMIT 1", [email]);
        let userId;
        ...
        if (rows.length === 0) {
          ...
          status,
          last_login_at
        ) VALUES (?, NULL, ?, 'google', ?, 'user', 'active', NOW())`, [email, resolvedDisplayName, providerId]);
